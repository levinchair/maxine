...
function flattenData(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    for (var j = 0; j < data[i].length; j++) {
      for(var k = 0; k < data[i][j].length; k++ ){
        for (var d = 0; d < dim; d++) result.vertices[i].push(data[i][j][k][d]);
      } 
    }
    // if (i > 0) {
  ...
  //if(result.vertices.length > 1) console.log("flattened vertices: " + JSON.stringify(result.vertices));
  //console.log(JSON.stringify(result.holes));
  return result;
}
